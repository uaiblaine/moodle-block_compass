...
$string['cachestores_desc'] = 'Compass keeps four application caches (coursemeta, categorymeta, inventory and details) and is designed for very large sites. Map the four definitions to a shared in-memory store, Redis by preference, under Site administration > Plugins > Caching > Configuration. Without a shared in-memory store the plugin still works, but performance can be severely degraded: every Dashboard visit falls back to the file store of each web node.';
